...
import { ThreadError, ThreadLayout } from '@/app/(dashboard)/projects/[projectId]/thread/_components';
...
export function ThreadComponent({ projectId, threadId, compact = false, configuredAgentId }: ThreadComponentProps) {
  ...
  // Billing disabled - removed upgrade dialog state
  ...
  // Billing disabled - removed upgrade dialog ref

  useEffect(() => {
    // Billing disabled - no upgrade dialog checks
    // Removed upgrade dialog logic completely
  }, [subscriptionData, subscriptionStatus, initialLoadCompleted]);

  // Billing disabled - removed upgrade dialog logic
  ...
  if (compact) {
    return (
      <>
        ...
        {/* Billing disabled - no upgrade dialog */}
