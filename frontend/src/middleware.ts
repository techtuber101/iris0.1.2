...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    if (!BILLING_ROUTES.includes(pathname) && pathname !== '/') {
      ...
      if (!accounts) {
        // Redirect to dashboard if no account exists
        const url = request.nextUrl.clone();
      ...
      if (!creditAccount) {
        // Redirect to dashboard if no credit account exists
        const url = request.nextUrl.clone();
        url.pathname = '/dashboard';
        return NextResponse.redirect(url);
      }
      ...
      if (!hasTier && !hasActiveTrial && !trialConverted) {
        // Redirect to dashboard for all users regardless of trial status
        const url = request.nextUrl.clone();
        url.pathname = '/dashboard';
        return NextResponse.redirect(url);
      } else if ((trialExpired || trialConverted) && !hasTier) {
        // Redirect to dashboard for expired/converted trials without tier
        const url = request.nextUrl.clone();
        url.pathname = '/dashboard';
        return NextResponse.redirect(url);
      }
