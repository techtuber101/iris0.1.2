...
def get_openrouter_fallback(model_name: str) -> Optional[str]:
    """Get Gemini fallback model for a given model name."""
    # Skip if already using a fallback model
    if model_name.startswith("openrouter/") or model_name.startswith("fallback/"):
        return None
    
    # Only provide Gemini-to-Gemini fallbacks since user only has Gemini API keys
    fallback_mapping = {
        "gemini/gemini-2.5-pro": "gemini/gemini-2.5-flash",
        "gemini/gemini-2.5-flash": "gemini/gemini-2.5-flash-lite",
        "gemini-2.5-pro": "gemini-2.5-flash",
        "gemini-2.5-flash": "gemini-2.5-flash-lite",
        "gemini-2.5-flash-lite": None,  # No fallback for lite version
    }
    ...
    # Default Gemini fallbacks (only if it's a Gemini model)
    if "gemini" in model_name.lower() or "google" in model_name.lower():
        if "pro" in model_name.lower():
            return "gemini-2.5-flash"
        elif "flash" in model_name.lower() and "lite" not in model_name.lower():
...
def _add_fallback_model(params: Dict[str, Any], model_name: str, messages: List[Dict[str, Any]]) -> None:
    """Add Gemini fallback model to the parameters."""
    ...
    if fallback_model:
        ...
        logger.debug(f"Added Gemini fallback for model: {model_name} to {fallback_model}")
...
def prepare_params(
    ...
    # Enhanced logging for title generation
    is_title_generation = any("title" in str(msg.get('content', '')).lower() or "brief" in str(msg.get('content', '')).lower() for msg in messages)
    
    if is_title_generation:
        logger.info(f"🎯 MODEL_RESOLVE: Starting model resolution for title generation")
        logger.info(f"🎯 MODEL_RESOLVE: Original model name: '{model_name}'")
    
    resolved_model_name = model_manager.resolve_model_id(model_name)
    
    if is_title_generation:
        logger.info(f"🎯 MODEL_RESOLVE: Resolved model name: '{resolved_model_name}'")
        logger.info(f"🎯 MODEL_RESOLVE: Model resolution successful: {model_name} -> {resolved_model_name}")
    ...
    if is_title_generation:
        logger.info(f"🎯 MODEL_RESOLVE: Final parameters prepared for {resolved_model_name}")
...
async def make_llm_api_call(
    ...
    """
    # Enhanced logging for title generation calls
    is_title_generation = any("title" in str(msg.get('content', '')).lower() or "brief" in str(msg.get('content', '')).lower() for msg in messages)
    
    if is_title_generation:
        logger.info(f"🎯 LLM_CALL: Title generation API call initiated")
        logger.info(f"🎯 LLM_CALL: Model: {model_name}")
        logger.info(f"🎯 LLM_CALL: Temperature: {temperature}")
        logger.info(f"🎯 LLM_CALL: Max tokens: {max_tokens}")
        logger.info(f"🎯 LLM_CALL: Stream: {stream}")
        logger.info(f"🎯 LLM_CALL: Messages count: {len(messages)}")
        
        for i, msg in enumerate(messages):
            role = msg.get('role', 'unknown')
            content = msg.get('content', '')
            logger.info(f"🎯 LLM_CALL: Message {i+1} - Role: {role}, Content: '{content}'")
    ...
    if is_title_generation:
        logger.info(f"🎯 LLM_CALL: Prepared parameters: {params}")
    ...
    try:
        if is_title_generation:
            logger.info(f"🎯 LLM_CALL: Making API call to {model_name}...")
        
        response = await provider_router.acompletion(**params)
        
        if is_title_generation:
            logger.info(f"🎯 LLM_CALL: ✅ Received response from {model_name}")
            logger.info(f"🎯 LLM_CALL: Response type: {type(response)}")
            logger.info(f"🎯 LLM_CALL: Response object: {response}")
        ...
        if is_title_generation:
            logger.error(f"🎯 LLM_CALL: ❌ API call failed for {model_name}: {str(e)}")
            logger.error(f"🎯 LLM_CALL: Exception type: {type(e)}")
        
        # Check if this is a Gemini/VertexAI overload error that should trigger fallback
        error_str = str(e)
        if ("VertexAIException" in error_str or 
            "model is overloaded" in error_str.lower() or
            "ServiceUnavailableError" in error_str or
            "MidStreamFallbackError" in error_str or
            "503" in error_str or
            "UNAVAILABLE" in error_str or
            "overloaded" in error_str.lower()):
            logger.warning(f"🔄 Gemini overload or mid-stream fallback detected, fallback should be triggered: {error_str}")
            # Don't wrap this error - let it propagate so the fallback mechanism can catch it
            raise
