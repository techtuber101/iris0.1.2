...
class ResponseProcessor:
    ...
    async def process_streaming_response(
        ...
        try:
            ...
            if (not "AnthropicException - Overloaded" in str(e) and 
                not "VertexAIException" in str(e) and 
                not "model is overloaded" in str(e).lower() and
                not "ServiceUnavailableError" in str(e) and
                not "MidStreamFallbackError" in str(e) and
                not "503" in str(e) and
                not "UNAVAILABLE" in str(e) and
                not "overloaded" in str(e).lower()):
                err_msg_obj = await self.add_message(
                    thread_id=thread_id, type="status", content=err_content, 
                ...
                logger.error(f"Model overload/service unavailable/mid-stream fallback detected - Falling back to Gemini Flash: {str(e)}", exc_info=True)
                self.trace.event(name="model_overload_service_unavailable_midstream_fallback_detected", level="ERROR", status_message=(f"Model overload/service unavailable/mid-stream fallback detected - Falling back to Gemini Flash: {str(e)}"))
